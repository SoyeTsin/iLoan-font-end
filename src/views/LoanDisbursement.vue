        ...
        <a-row type="flex" justify="space-between">
          ...
          <a-col :span="7">
            ...
            <a-form-item
              ...
              :extra="$t('loan.support_file_type')"
            >
              <a-upload
                :multiple="false"
                :file-list="fileList"
...
export default {
  ...
  methods: {
    moment,
    changeAmt(e) {
      this.form.setFieldsValue({ PrincipalAmount: e })
    },
    changeStatus(e) {
      if (e.target.value === '01') {
        this.form.setFieldsValue({ TranAmt: this.$route.query.AvailableQuota, PrincipalAmount: this.$route.query.AvailableQuota })
      } else {
        this.form.setFieldsValue({ TranAmt: '', PrincipalAmount: '' })
      }
      this.LoanStatus = e.target.value
    },
    checkAmt() {
      const amt = Number(this.$route.query.AvailableQuota);
        ...
        if (!err) {
          this.upFile().then((fileKey) => {
            if (fileKey) {
              ...
              Loan.ManualDisbursementApply(param).then((res) => {
                if (res.Status === "ok") {
                  this.$message.success(this.$t('loan.apply_disbursement_success'));
                  this.$router.push({
    ...
    beforeUpload(e) {
      ...
      const outoflimit = e.size / 1024 > 500;
